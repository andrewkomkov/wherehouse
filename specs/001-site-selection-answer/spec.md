...
| **Per-category ring radius, land use, competitor quality** | Each is a real modelling improvement and none fit before 21 July. The score's honesty comes from stating what it omits, not from omitting less. |
| **A "low rent" factor** — searched for properly on 20 July, **cut on evidence** | The designer's comp has a Low rent slider; there is no data behind it in any of the three cities, and the comp fakes it with a Gaussian. What actually exists, each endpoint fetched and read: **Berlin** `wohnlagenadr2026` (400,505 addresses, `Datenlizenz Deutschland – Zero – 2.0`) — but `wol` is *Wohnlage*, a **3-value category** of residential location quality, an input to the Mietspiegel, not a price; `mittel` alone is ~49% of addresses, so the layer is three flat colours and half the map is one shade. **Amsterdam** CBS `GemiddeldeWOZWaardeVanWoningen_39` per buurt — but WOZ is **property sale valuation**, not rent, and shares no scale with Berlin's categories. **Belgrade** — nothing (verified in Cyrillic; the portal has 3,459 datasets and Latin queries return 0, which nearly produced a false negative). Three different proxies in three different units, one empty. Rent is the first thing a site-selection juror probes, so a plausible-but-wrong rent layer would discredit the population and GAP layers that are real. |
| **"Footfall"** | We have **residents** (Kontur), not foot traffic. The slider is renamed *Residents* rather than cut — the data is real, the comp's label was not. |
